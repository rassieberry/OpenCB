...
wxDEFINE_EVENT(MYTHREAD_COMPLETED, wxThreadEvent);
wxDEFINE_EVENT(MYTHREAD_UPDATE, wxThreadEvent);
...
public:
    ...
    MyTextCtrl* log_spam;
    MyTextCtrl* program_text;
    MyTextCtrl* user_text;
...
public:
    ...
    {
        main_thread_frame_handler = handler;
        //num_sub_threads = 0;
        //sub_threads_running = false;
        //sub_threads_array = &sub_threads_array_obj;
    }
    ~MyThread();

    // commenting these out for now while i try to get the sub threads running in another context.
    //void RunLifecycle();
    //void DoExistingSubThread();
    //void DoImaginingSubThread();
    //void DoThinkingSubThread();
    //void FinishSubThreads();

    wxMutex *main_thread_mutex = new wxMutex();

    MyThread* main_thread_handler;

    //wxArrayThread sub_threads_array_obj;
    //wxArrayThread* sub_threads_array;
    //int num_sub_threads;
    //bool sub_threads_running;

    ocb::Lifecycle Angie = new ocb::Lifecycle(true);

    MyFrame* main_thread_frame_handler;

protected:
    virtual ExitCode Entry();
};

class MySubThread : public wxThread
{
public:
    MySubThread(MyThread* handler)
        : wxThread(wxTHREAD_JOINABLE)
    {
        sub_thread_handler = handler;
        sub_thread_frame_handler = handler->main_thread_frame_handler;
        //thread_type = 0;
    }
    ~MySubThread();
    /*
    ...
    ocb::Lifecycle t_angie;
    int thread_type;
    */
protected:
    virtual ExitCode Entry();
    MyThread* sub_thread_handler;
    MyFrame* sub_thread_frame_handler;
};
...
public:
    ...
    bool do_subthread_loop;

    MyThread *main_thread;
    ...
    {
        static int s_n = 0;
        m_panel->user_text->AppendText(wxString::Format("Line %d\n", ++s_n));
    }
    ...
    void OnGetLine(wxCommandEvent& WXUNUSED(event))
    {
        ...
        wxMessageBox(m_panel->user_text->GetLineText(nLine));
    }
    ...
    void OnGetLineLength(wxCommandEvent& WXUNUSED(event))
    {
        ...
        wxMessageBox(wxString::Format("Length of line %i is:%i",
            (int)nLine,
            m_panel->user_text->GetLineLength(nLine))
        );
    }
    ...
    {
        m_panel->user_text->SetValue("Hello, world! (what else did you expect?)");
    }

    void OnChangeText(wxCommandEvent& WXUNUSED(event))
    {
        m_panel->user_text->ChangeValue("Changed, not set: no event");
    }
    ...
    wxCriticalSection main_threadCS;
...
private:
    ...
    {
        wxTextCtrl* const text = m_panel->user_text;
...
bool MyApp::OnInit()
{
    ...
    MyFrame* frame = new MyFrame("Text wxWidgets sample", 500, 500);

    wxMenu* file_menu = new wxMenu;
    file_menu->Append(TEXT_RUN, "Run");
    file_menu->Append(TEXT_LOAD, "Stop");
...
EVT_THREAD(MYTHREAD_UPDATE, MyFrame::OnThreadUpdate)
EVT_THREAD(MYTHREAD_COMPLETED, MyFrame::OnThreadCompletion)
wxEND_EVENT_TABLE()
...
//    m_textrich->SetStyle(endPos - 4, endPos - 2, attr);
    ...
    log_spam = new MyTextCtrl(this, wxID_ANY, "", wxPoint(500, 10), wxSize(200, 300), wxTE_RICH | wxTE_MULTILINE);

    program_text = new MyTextCtrl(this, wxID_ANY, "", wxPoint(255, 10), wxSize(200, 300), wxTE_RICH | wxTE_MULTILINE);

    user_text = new MyTextCtrl(this, wxID_ANY, "", wxPoint(10, 10), wxSize(200, 300), wxTE_RICH | wxTE_MULTILINE);

    // lay out the controls
    wxBoxSizer* column1 = new wxBoxSizer(wxVERTICAL);
    column1->Add(user_text, 1, wxALL | wxEXPAND, 10);

    wxBoxSizer* column2 = new wxBoxSizer(wxVERTICAL);
    column2->Add(program_text, 1, wxALL | wxEXPAND, 10);

    wxBoxSizer* column3 = new wxBoxSizer(wxVERTICAL);
    column3->Add(log_spam, 1, wxALL | wxEXPAND, 10);

    wxBoxSizer* row1 = new wxBoxSizer(wxHORIZONTAL);
    row1->Add(column1, 1, wxALL | wxEXPAND, 10);
...
{
    user_text->SetInsertionPointEnd();
    user_text->SetFocus();
}
...
EVT_MENU(TEXT_RUN, MyFrame::OnFileRun)
...
EVT_BUTTON(RunButton, MyFrame::OnRunButton)
EVT_BUTTON(StopButton, MyFrame::OnStopButton)
EVT_BUTTON(StartButton, MyFrame::OnStartButton)
    ...
    : wxFrame(NULL, wxID_ANY, title, wxPoint(x, y))
{
    ...
    // run and stop buttons here
    m_panel = new MyPanel(this, 10, 10, 800, 400);
    m_panel->GetSizer()->Fit(this);
    wxStaticBoxSizer* runSizer =
        new wxStaticBoxSizer(wxVERTICAL, m_panel);
    wxButton* RunProgram =
        new wxButton(m_panel, RunButton, "Run");
    wxButton* StopProgram =
        new wxButton(m_panel, StopButton, "Stop", wxPoint(80, 0));
    runSizer->Add(RunProgram, 1, wxEXPAND | wxALL, 2);
    runSizer->Add(StopProgram, 1, wxEXPAND | wxALL, 2);
    main_thread_running = false;
    do_subthread_loop = false;
}
...
void MyFrame::OnSetEditable(wxCommandEvent& WXUNUSED(event))
{
    ...
    m_panel->user_text->SetEditable(s_editable);
}

void MyFrame::OnSetEnabled(wxCommandEvent& WXUNUSED(event))
{
    bool enabled = m_panel->user_text->IsEnabled();

    m_panel->user_text->Enable(enabled);
    m_panel->program_text->Enable(enabled);
    m_panel->log_spam->Enable(enabled);
}

void MyFrame::OnFileRun(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        std::cout << "Error: tried to create an extra main thread.";
        std::cout << std::endl;
    }
}
...
void MyFrame::OnRunButton(wxCommandEvent& WXUNUSED(event))
{
    if (!main_thread_running)
    {
        DoStartThread();
    ...
    {
        //main_thread->RunLifecycle();
    }

}

void MyFrame::OnStopButton(wxCommandEvent& WXUNUSED(event))
{
    StopThread();
}

void MyFrame::OnStartButton(wxCommandEvent& WXUNUSED(event))
{
...
void MyFrame::DoStartThread()
{
    if (main_thread_running)
    {
        std::cout << ("Error: Main thread is already running.");
        std::cout << std::endl;
    }
    else
    {
...
void MyFrame::StartThread()
{
    main_thread = new MyThread(this);
    std::cout << "Creating main thread.";
    std::cout << std::endl;
    main_thread->main_thread_mutex->Lock();
    main_thread->main_thread_handler = main_thread;
    main_thread->Create();
    main_thread_running = true;
    main_thread->main_thread_mutex->Unlock();
    main_thread->Run();
}

wxThread::ExitCode MyThread::Entry()
{
    if (!main_thread_frame_handler->main_thread_running)
    {
        Angie.arrive();
        main_thread_frame_handler->main_thread_running = true;
    }
    while (!TestDestroy())
    {
        //Angie.exist();
        //RunLifecycle();
        //sub_threads_array->at(1)->Wait();
        //sub_threads_array->at(2)->Wait();
        //sub_threads_array->at(3)->Wait();
        //FinishSubThreads();
        //wxQueueEvent(main_thread_frame_handler, new wxThreadEvent(MYTHREAD_UPDATE));
    }
    wxQueueEvent(main_thread_frame_handler, new wxThreadEvent(MYTHREAD_COMPLETED));

    return (wxThread::ExitCode)0;
}

wxThread::ExitCode MySubThread::Entry()
{/*
    while (!TestDestroy())
    {
        wxQueueEvent(sub_thread_frame_handler, new wxThreadEvent(MYTHREAD_UPDATE));
        if (thread_type == ocb::EXISTING_SUBTHREAD)
        {
            DoExisting();
        }
        else if (thread_type == ocb::IMAGINING_SUBTHREAD)
        {
            DoImagining();
        }
        else if (thread_type == ocb::THINKING_SUBTHREAD)
        {
            DoThinking();
        }
    }*/
    wxQueueEvent(sub_thread_frame_handler, new wxThreadEvent(MYTHREAD_COMPLETED));
...
MyThread::~MyThread()
{
    wxCriticalSectionLocker enter(main_thread_frame_handler->main_threadCS);

    main_thread_frame_handler->main_thread = NULL;
}


MySubThread::~MySubThread()
{
    wxCriticalSectionLocker enter(sub_thread_frame_handler->main_threadCS);

    sub_thread_handler->main_thread_handler = NULL;
}

void MyFrame::StopThread()
{
    wxCriticalSectionLocker enter(main_threadCS);
    if (main_thread)
    {
        wxLogDebug("Deleting thread...");

        if (main_thread->Delete() != wxTHREAD_NO_ERROR) 
        {
            wxLogError("Error: unable to delete thread.");
        }
        else
        {
            main_thread_running = false;
...
void MyFrame::OnThreadCompletion(wxThreadEvent& evt)
{
    wxLogDebug("Thread exited successfully.");
}

void MyFrame::OnThreadUpdate(wxThreadEvent& evt)
{
    wxLogDebug("Thread updated.");
}
...
// actions that are part of the main while loop that runs the threads
/*
void MySubThread::DoExisting()
{
    ...
    //this.Angie.Lifecycle_copy(t_angie);
}*/
/*
void MyThread::DoExistingSubThread()
{
    MySubThread* existing_thread;
    main_thread_mutex->Lock();
    existing_thread = new MySubThread(main_thread_handler);
    existing_thread->thread_type = ocb::EXISTING_SUBTHREAD;
    this->sub_threads_array->Add(existing_thread, existing_thread->thread_type);
    this->num_sub_threads++;
    existing_thread->Create();
    existing_thread->t_angie.Lifecycle_copy(this->Angie);
    main_thread_mutex->Unlock();
    existing_thread->Run();

    std::cout << "Action thread finished.";
    std::cout << std::endl;
...
void MyThread::DoImaginingSubThread()
{
    MySubThread* imagining_thread;
    main_thread_mutex->Lock();
    imagining_thread = new MySubThread(main_thread_handler);
    imagining_thread->thread_type = ocb::IMAGINING_SUBTHREAD;
    this->sub_threads_array->Add(imagining_thread, imagining_thread->thread_type);
    this->num_sub_threads++;
    imagining_thread->Create();
    imagining_thread->t_angie.Lifecycle_copy(this->Angie);
    main_thread_mutex->Unlock();
    imagining_thread->Run();

    std::cout << "Imagining thread finished.";
    std::cout << std::endl;
...
void MyThread::DoThinkingSubThread()
{
    MySubThread* thinking_thread;
    main_thread_mutex->Lock();
    thinking_thread = new MySubThread(main_thread_handler);
    thinking_thread->thread_type = ocb::THINKING_SUBTHREAD;
    this->sub_threads_array->Add(thinking_thread, thinking_thread->thread_type);
    this->num_sub_threads++;
    thinking_thread->Create();
    thinking_thread->t_angie.Lifecycle_copy(this->Angie);
    main_thread_mutex->Unlock();
    thinking_thread->Run();

    std::cout << "Thinking thread finished.";
    std::cout << std::endl;
    return;
}

void MySubThread::DoActionPlan()
{
    t_angie.live();
}

void MySubThread::DoActionNoPlan()
{
    t_angie.live_no_plan();
}

void MySubThread::DoImagining()
{
    t_angie.imagining();
}

void MySubThread::DoThinking()
{
    t_angie.thinking();
}

void MySubThread::DoFallAsleep()
{
    t_angie.fall_asleep();
}

void MySubThread::DoSleep()
{
    t_angie.sleep();
}

void MySubThread::DoWakeUp()
{
    t_angie.wake_up();
}


void MyThread::RunLifecycle()
{
    bool check;
    if (main_thread_mutex->TryLock() == wxMUTEX_NO_ERROR)
    {
        check = sub_threads_running;
        main_thread_mutex->Unlock();
    }
    else
    {
        std::cout << "Error: main thread already running.";
        std::cout << std::endl;
        return;
    }
    if (!check)
    {
        this->DoExistingSubThread();
        this->DoImaginingSubThread();
        this->DoThinkingSubThread();
        sub_threads_running = true;
    }
    else
    {
        std::cout << "Error: Sub threads already running.";
        std::cout << std::endl;
    }
}

void MyThread::FinishSubThreads()
{
    if (sub_threads_running)
    {
        wxThread::ExitCode st_error1 = sub_threads_array->at(ocb::EXISTING_SUBTHREAD)->Wait();
        sub_threads_array->at(ocb::EXISTING_SUBTHREAD)->Delete();
        sub_threads_array->at(ocb::EXISTING_SUBTHREAD) = NULL;
        num_sub_threads--;
        wxThread::ExitCode st_error2 = sub_threads_array->at(ocb::IMAGINING_SUBTHREAD)->Wait();
        sub_threads_array->at(ocb::IMAGINING_SUBTHREAD)->Delete();
        sub_threads_array->at(ocb::IMAGINING_SUBTHREAD) = NULL;
        num_sub_threads--;
        wxThread::ExitCode st_error3 = sub_threads_array->at(ocb::THINKING_SUBTHREAD)->Wait();
        sub_threads_array->at(ocb::THINKING_SUBTHREAD)->Delete();
        sub_threads_array->at(ocb::THINKING_SUBTHREAD) = NULL;
        num_sub_threads--;
        /*
        delete& sub_threads_array->at(1);
        delete& sub_threads_array->at(2);
        delete& sub_threads_array->at(3);
        sub_threads_running = false;
    }
    else
    {
        std::cout << "Error: cannot finish sub threads as none were running.";
        std::cout << std::endl;
    }
}*/